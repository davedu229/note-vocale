...
const NavItem = ({ to, icon: Icon, label }) => (
    <NavLink
        to={to}
        className={({ isActive }) => `
            relative flex flex-col items-center gap-1 px-3 py-2 rounded-2xl transition-all duration-200 active:scale-95
...
const SidebarItem = ({ to, icon: Icon, label }) => (
    <NavLink
        ...
        className={({ isActive }) => `
            ...
            ${isActive ? 'bg-primary/10 text-white' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}
        `}
    >
        {({ isActive }) => (
            <>
